    ...
    event_loop::set_timeout(
        [cb]() {
          ...
          if (status) {
            ...
            auto status = pressure.startPressure(
                3); // The parameter is the oversampling setting, from 0 to 3
            ...
            if (status) {
              event_loop::set_timeout(
                  [cb, temperature]() {
                    ...
                    if (status) {
                      DBG_OUT << "absolute pressure: " << presure << " mb"
                              << std::endl;
                      cb(tmp, presure, true);
                    } else {
...
void BH1750_get(std::function<void(const float lux, bool status)> cb) {
  if (lightMeter.measurementReady()) {
    ...
    event_loop::set_timeout(
        [cb = std::move(cb)]() { BH1750_get(std::move(cb)); }, 200ms);
  }
}
void battery_get(std::function<void(const float volt, bool status)> cb) {
  ...
  event_loop::set_timeout(
      [first, cb = std::move(cb)]() {
        ...
        const auto volt =
            static_cast<float>(val) * (130 + 220 + 100) / 100 / 1024;
